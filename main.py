...
df = pd.read_csv('marketing_campaign_cleaned.csv', index_col=[0])
...
st.plotly_chart(fig)
image = Image.open('model_sum.png')
st.image(image)
st.markdown('''
...
image = Image.open('parent_odds.png')
st.image(image)
...
image = Image.open('camp_spend.png')
st.image(image)
